...
public class TankBot extends RobotGlobal {
	...
	static boolean willHitFarm() throws GameActionException {
		for (MapLocation fLoc: farmLocs) {
			if (fLoc == null) continue;
			if (Clock.getBytecodeNum() > 14000) {
				System.out.println("Bytecodes!!!");
				break;
			}
			System.out.println("check " + fLoc);
			if (willConeOfFireHitCircle(attackLoc, fLoc, 6f)) return true;
		}
		return false;
	}
	...
	public static void turn() throws GameActionException {
		...
		for (MapLocation f: farmLocs) {
			if (f == null) continue;
			rc.setIndicatorDot(f, 55, 255, 55);
		}
